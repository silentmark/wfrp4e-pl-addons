    ...
    setup() {
        if (game.settings.get("wfrp4e-pl-addons", "templateCollision")) {
            ...
            AreaHelpers.isInTemplate = function (tokenObject, templateDocument, size) {
                ...
                let minimalRatio = game.settings.get("wfrp4e-pl-addons", "templateCollisionMinimalRatio") / 100;
            ...
            AreaHelpers.checkAreaUpdate = async function (template, update, options, user) {
                ...
                if (game.users.activeGM.id == game.user.id && !options?.skipAreaCheck) {
                    ...
                    let tokens = template.parent?.tokens.contents.filter(token => AreaHelpers.isInTemplate(token.object, template) || token.actor?.effects.find(e => e.system.sourceData.area == template.uuid));                    
            ...
            AreaHelpers.checkTokenAreaEffects = async function (token, newCenter) {
                ...
                let scene = token.parent;
                let inAreas = scene.templates.contents.filter(t => AreaHelpers.isInTemplate(newCenter || token.object.center, t, { width: token.width, height: token.height }));
