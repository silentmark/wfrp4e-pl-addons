...
class SocketTests {
  ...
  ready() {
    ...
    Reflect.defineProperty(ActorWFRP4e.prototype, '_setupSocketTest', {
      async value(dialogData, dialogClassName) {
        ...
        if (owner.id != game.user.id && isSocketTest) {
          ...
          const testData = await SocketHandlers.executeOnUserAndWait(owner.id, 'setupSocketTest', payload);
          msg.delete();
          if (testData) {
    ...
    Reflect.defineProperty(ActorWFRP4e.prototype, 'setupCharacteristic', {
      async value(characteristic, options = {}) {
        const dialogData = {
