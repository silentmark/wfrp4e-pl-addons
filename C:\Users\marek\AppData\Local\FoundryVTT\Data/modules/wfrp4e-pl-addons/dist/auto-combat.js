  ...
  setup() {
    if (game.settings.get('wfrp4e-pl-addons', 'autoCombat.Enabled')) {
      ...
      Hooks.on('updateCombat', async(combat, updateData) => {
        ...
        const arr = combat.combatants.filter(x => !x.hidden && !Number.isFinite(x.initiative));
        if (arr.length > 0) {
          return;
        }
        if (combat.current && combat.active && combat.current.tokenId) {
          const token = game.scenes.active.tokens.get(combat.current.tokenId);
          const tokenObj = canvas.tokens.get(combat.current.tokenId);
          const { actor } = token;
          ...
          if (distance <= 2) {// melee
            ...
            if (stuffToUse.length > 0) {
              ...
              for (let i = 1; i < stuffToUse.length; i++) {
                ...
                const newSkillToUse = newSetupItem.skillToUse;
                if (newSkillToUse) {
                  newMax = newSkillToUse.system.total.value;
                } else {
      ...
      Hooks.on('renderChatMessage', async(app, html, messageData) => {
        ...
        if (castTest?.constructor?.name == 'WomCastTest' && castTest.result.castOutcome == 'success') {
          ...
          newMessage.find('.card-apply-spell').click(async() => {
            ...
            const { item } = castTest;
            const { actor } = castTest;
            // TODO: fix
            console.warn('Auto-combat: Effect processing needs implementation');
        ...
        if (game.ready && app.system.constructor.name == 'OpposedHandlerMessage' && app.system.opposedData.attackerMessageId) {
          ...
          if (postFunction == 'WeaponTest' || postFunction == 'TraitTest') {
            ...
            if (msg.system.test.context.speaker.actor) {
              ...
              const speakerToken = canvas.tokens.get(msg.system.test.context.speaker.token);
            ...
            if (msg.system.test.context.targets
              ...
              const target = game.scenes.active.tokens.get(app.system.opposedData.targetSpeakerData.token);
              if (target.actor?.flags?.wfrp4e?.autoCombat && target.actor.flags.wfrp4e.autoCombat > 0) {
                ...
                if (item.attackType != 'ranged') {
                  ...
                  const { opposedTest } = resultMessage.system;
