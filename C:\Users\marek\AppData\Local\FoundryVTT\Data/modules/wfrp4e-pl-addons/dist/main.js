  ...
  setup() {
    if (game.settings.get('wfrp4e-pl-addons', 'templateCollisionMethod') != 'default') {
      ...
      AreaHelpers.isInTemplate = function(tokenDocument, templateObject) {
        const collisionMethod = game.settings.get('wfrp4e', 'templateCollisionMethod');
        const minimalRatio = 0.25;
        if (!templateObject.shape) {
          const { x, y } = templateObject.document;
          let { direction, distance } = templateObject.document;
        ...
        if (collisionMethod == 'centerPoint') {
          ...
          const { size } = tokenDocument.parent.dimensions;
          ...
          case 0: { // generic poly
            const { x } = templateObject;
            const { y } = templateObject;
      ...
      AreaHelpers.checkAreaUpdate = async function(template, update, options, user) {
        ...
        if (game.users.activeGM.id == game.user.id && !options?.skipAreaCheck) {
          ...
          const tokens = template.parent?.tokens.contents.filter(token => AreaHelpers.isInTemplate(token, template) || token.actor.effects.find(e => e.system.sourceData.area == template.uuid));
      ...
      AreaHelpers.checkTokenAreaEffects = async function(token, newCenter) {
        ...
        if (newCenter) {
          ...
          const scene = token.parent;
          const inAreas = scene.templates.contents.filter(t => AreaHelpers.isInTemplate(token, t));
          const effects = Array.from(token.actor.effects);
          const areaEffects = [];
          inAreas.forEach(area => {
            const areaEffect = area.areaEffect();
            const auraData = area.getFlag('wfrp4e', 'aura');
