...
class Miscasts {
  ...
  setup() {
    if (game.settings.get('wfrp4e-pl-addons', 'alternativeMiscasts.Enable')) {
      ...
      Reflect.defineProperty(TestWFRP.prototype, '_handleMiscasts', { value(miscastCounter)  {
        const maxRandom = 25;
        let mis = false;
        ...
        if (mis) {
          ...
          if (this.item.system.ritual.value) {
            const spellCn = Math.floor(Math.max(Number.parseInt(this.item.cn.SL, 10), Number.parseInt(this.item.cn.value, 10)) / 3);
            ...
            const spellCn = Math.max(Number.parseInt(this.item.cn.SL, 10), Number.parseInt(this.item.cn.value, 10)) * 3;
          ...
          if (table) {
            ...
            if (Number.parseInt(new Roll(formula).roll({ async: false }).result, 10) > 50) {
