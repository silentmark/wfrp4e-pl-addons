...
class AutoCounterSpell {
  ...
  setup() {
    if (game.settings.get('wfrp4e-pl-addons', 'counterSpells.Enable')) {
      const updateCombatCasters = async function(combat, updateData) {
        if (game.user.isGM && combat.active) {
          ...
          for (let i = 0; i < actorIds.length; i++) {
            ...
            const skills = actor.itemTags['skill'].filter(x=> x.name === 'Język (Magiczny)');
            if (skills.length == 0) {
              continue;
            }
      ...
      Hooks.on('preUpdateCombat', async(combat, updateData) => {
        if (game.user.isGM) {
          if (combat.round != 0 && combat.turns && combat.active) {
            if (combat.current.turn > -1 && combat.current.turn == combat.turns.length - 1) {
              ...
              if (updateData.flags && updateData.flags['wfrp4e-pl-addons']) {
                const moduleFlags = updateData.flags['wfrp4e-pl-addons'];
      ...
      Hooks.on('wfrp4e:rollCastTest', async(castData, chatData) => {
      ...
      Hooks.on('wfrp4e:opposedTestResult', async(opposedData, attackerTest, defenderTest) => {
        ...
        if (opposedData.data.attackerTestData.preData.rollClass == 'CastTest'
          ...
          const { messageId } = attackerTest.data.context;
          const message = game.messages.get(messageId);
          const { test } = message.system;
...
export { AutoCounterSpell as default };
