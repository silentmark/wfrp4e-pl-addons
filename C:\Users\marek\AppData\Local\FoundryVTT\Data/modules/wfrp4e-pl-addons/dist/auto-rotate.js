...
class AutoRotate {
  ...
  calculateRotation = function(args) {
    ...
    if (args.target && (args.item.type === 'weapon' || args.item.type === 'trait') && args.item.attackType == 'melee') {
      ...
      if (attackingToken == null || targetToken == null) return;
      ...
      const attackAngle =
                Math.abs(
                  ...
                  ) * (180 / Math.PI)) - targetToken.document.rotation)
                    % 360);
      if ((attackAngle > frontRange1Min && attackAngle < frontRange1Max) || attackAngle > frontRange2Min && attackAngle < frontRange2Max) ; else if ((attackAngle >= rearRange1Min && attackAngle <= rearRange1Max) || attackAngle >= rearRange2Min && attackAngle <= rearRange2Max) {
        const newScript = {
  ...
  setup() {
    if (game.settings.get('wfrp4e-pl-addons', 'autoRotate.Enable')) {
      Hooks.on('wfrp4e:rollWeaponTest', (weaponTest, cardOptions) => {
        if (weaponTest.context.defending) return;
        const attackToken = canvas.tokens.get(cardOptions.speaker.token);
        const defenderToken = canvas.tokens.get(weaponTest.context.targets[0]?.token);
